const MOVIES = [
  { id:"1", title:"Dune: Part Three",   genre:"Sci-Fi / Epic",      rating:9.1, votes:"284K", duration:"2h 52m", badge:"4K",   emoji:"🏜️", stdPrice:220, premPrice:350, desc:"The final chapter of Paul Atreides' saga across Arrakis.", cast:["Timothée Chalamet","Zendaya","Rebecca Ferguson"], director:"Denis Villeneuve", language:"English",          release:"2026-01-10", tags:["Epic","Sci-Fi","Drama"] },
  { id:"2", title:"The Last Frontier",  genre:"Action / Thriller",  rating:8.4, votes:"191K", duration:"2h 18m", badge:"IMAX", emoji:"🚀", stdPrice:280, premPrice:420, desc:"An elite team races across three continents to stop a rogue satellite network.", cast:["Chris Hemsworth","Ana de Armas","Idris Elba"], director:"Chad Stahelski", language:"English", release:"2026-02-14", tags:["Action","Thriller","Spy"] },
  { id:"3", title:"Neon Shadows",       genre:"Neo-Noir / Mystery", rating:8.9, votes:"142K", duration:"1h 58m", badge:"",     emoji:"🌆", stdPrice:200, premPrice:320, desc:"A disgraced detective unravels a conspiracy in a rain-drenched neon city.", cast:["Oscar Isaac","Cate Blanchett","Mahershala Ali"], director:"Park Chan-wook", language:"English/Korean", release:"2026-02-28", tags:["Neo-Noir","Mystery","Crime"] },
  { id:"4", title:"Crimson Dynasty",    genre:"Historical / Drama", rating:8.6, votes:"178K", duration:"3h 05m", badge:"NEW",  emoji:"🏯", stdPrice:200, premPrice:320, desc:"A sweeping epic of four generations torn apart by war and betrayal.", cast:["Tony Leung","Zhang Ziyi","Andy Lau"], director:"Zhang Yimou", language:"Mandarin", release:"2026-03-05", tags:["Historical","Drama","War"] },
  { id:"5", title:"Quantum Break",      genre:"Sci-Fi / Action",    rating:7.8, votes:"156K", duration:"2h 10m", badge:"IMAX", emoji:"⚡", stdPrice:280, premPrice:420, desc:"A physicist discovers time manipulation — and finds someone else already knows.", cast:["Florence Pugh","Michael B. Jordan","Awkwafina"], director:"Gareth Edwards", language:"English", release:"2026-03-12", tags:["Sci-Fi","Action","Time Travel"] },
  { id:"6", title:"Ocean's Reckoning",  genre:"Comedy / Heist",     rating:8.2, votes:"203K", duration:"1h 56m", badge:"",     emoji:"🎰", stdPrice:200, premPrice:320, desc:"The world's most charming thieves return for one last job.", cast:["Ryan Gosling","Margot Robbie","Donald Glover"], director:"Steven Soderbergh", language:"English", release:"2026-03-15", tags:["Comedy","Heist","Crime"] },
  { id:"7", title:"Midnight in Kyoto",  genre:"Romance / Drama",    rating:8.7, votes:"119K", duration:"2h 02m", badge:"",     emoji:"🌸", stdPrice:200, premPrice:320, desc:"Two strangers spend one magical night in Kyoto during cherry blossom season.", cast:["Saoirse Ronan","Steven Yeun","Gemma Chan"], director:"Sofia Coppola", language:"English/Japanese", release:"2026-03-18", tags:["Romance","Drama","Art House"] },
  { id:"8", title:"Dark Matter",        genre:"Horror / Thriller",  rating:8.0, votes:"167K", duration:"1h 45m", badge:"NEW",  emoji:"👁️", stdPrice:200, premPrice:320, desc:"A family's dream home exists simultaneously in a dimension of living nightmares.", cast:["Lupita Nyong'o","Daniel Kaluuya","Anya Taylor-Joy"], director:"Jordan Peele", language:"English", release:"2026-03-17", tags:["Horror","Thriller","Psychological"] },
];
...
const Store = {
  ...
  toggleWishlist:(id)=>{ const w=Store.getWishlist(); const i=w.indexOf(id); i>=0?w.splice(i,1):w.push(id); Store.set("cx_wishlist",w); return i<0; }
};
...
function genTakenSeats(movieId,date,time){
  ...
  const taken=new Set();
  ROWS.forEach((r,ri)=>{ for(let i=1;i<=10;i++){ if(rng(seed*ri*13+i*7)<0.28) taken.add(r+i); } });
  return taken;
}
...
function renderNav(activePage){
  const pages=[{href:"index.html",label:"Home"},{href:"movies.html",label:"Movies"},{href:"offers.html",label:"Offers"},{href:"my-tickets.html",label:"My Tickets"}];
  const count=Store.getBookings().length;
  return `<header>
    <a href="index.html" class="logo">CINEMAX</a>
    <nav>${pages.map(p=>`<a href="${p.href}" class="${activePage===p.href?'active':''}">${p.label}</a>`).join("")}</nav>
    <div class="header-right">
      <a href="my-tickets.html" class="ticket-badge">
        ${count>0?`<span class="badge-count">${count}</span>`:""}
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 5v2M15 11v2M15 17v2M5 5h14a2 2 0 012 2v3a2 2 0 000 4v3a2 2 0 01-2 2H5a2 2 0 01-2-2v-3a2 2 0 000-4V7a2 2 0 012-2z"/></svg>
      </a>
    </div>
  </header>`;
}
...
const SHARED_CSS=`
  ...
  :root{--bg:#09090e;--surface:#111118;--card:#18181f;--card2:#1e1e27;--border:#252530;--accent:#e8c96d;--accent2:#ff6b6b;--accent3:#6de8c9;--text:#eeecf0;--muted:#6a6a88;--green:#4caf87;--purple:#8b5cf6;}
  *{margin:0;padding:0;box-sizing:border-box} html{scroll-behavior:smooth}
  ...
  .btn-gold{background:var(--accent);color:var(--bg)} .btn-gold:hover{background:#f0d880;transform:translateY(-2px);box-shadow:0 8px 24px rgba(232,201,109,.3)}
  .btn-outline{background:transparent;color:var(--text);border:1.5px solid var(--border)} .btn-outline:hover{border-color:var(--muted)}
  .movie-card{background:var(--card);border-radius:14px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:all .25s}
  ...
  .movie-info{padding:1rem} .movie-title-sm{font-weight:600;font-size:.92rem;margin-bottom:.35rem;line-height:1.3}
  ...
  .alert-bar.show{display:block} .alert-bar.err{background:rgba(255,107,107,.07);border-color:rgba(255,107,107,.3);color:var(--accent2)}
  ...
  .footer-links a:hover{color:var(--text)} .footer-copy{font-size:.73rem;color:var(--muted)}
  @media(max-width:768px){header{padding:1rem 1.2rem} nav{display:none} .section{padding:3rem 1.2rem} footer{padding:2rem 1.2rem;flex-direction:column;align-items:flex-start}}
`;
```
